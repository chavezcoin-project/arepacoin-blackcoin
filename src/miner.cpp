...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ArepacoinMiner : mined block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArepacoinMiner : ProcessBlock, block not accepted");
    }
...
bool CheckStake(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    LogPrintf("ArepacoinStaker: proof-of-stake found  \n  hash: %s\n  stake: %s\n  target: %s\n", hash.GetHex(), hashStake.GetHex(), hashTarget.GetHex());
    pblock->print();
    ...
    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ArepacoinStaker : minted block is stale");
        ...
        CValidationState state;
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArepacoinStaker : ProcessBlock, block not accepted");
    }

    return true;
}

void ArepacoinStaker(CWallet *pwallet)
{
    LogPrintf("ArepacoinStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the staking thread
    RenameThread("arepacoin-staker");
    ...
    try { while (true) {
        if (Params().NetworkID() != CChainParams::REGTEST) {
            ...
            while (vNodes.empty())
            {
                ...
                LogPrintf("ArepacoinStaker : Waiting for network online.\n");
        ...
        while (IsInitialBlockDownload()) 
        {
            ...
            LogPrintf("ArepacoinStaker : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }

        while (pwallet->IsLocked())
        {
            LogPrintf("ArepacoinStaker : Wallet is locked.\n");
            nLastCoinStakeSearchInterval = 0;
            MilliSleep(1000);
        }
        ...
        {
            LogPrintf("ArepacoinStaker : Chaintip < Last POW.\n");
            MilliSleep(60000);
        }
        ...
        {
            // LogPrintf("ArepacoinStaker : Failed to sign the new block.\n");
            MilliSleep(1000);
        }
    } }
    catch (boost::thread_interrupted)
    {
        LogPrintf("ArepacoinStaker terminated\n");
        throw;
    }
}

void static ArepacoinMiner(CWallet *pwallet)
{
    LogPrintf("ArepacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    try { while (true) {
        if (Params().NetworkID() != CChainParams::REGTEST) {
            ...
            // on an obsolete chain. In regtest mode we expect to fly solo.
            while (vNodes.empty()){
                LogPrintf("ArepacoinMiner : Waiting for network online.\n");
                MilliSleep(1000);
            }
        }
        ...
        {
            // Busy-wait for the download of the blockchain to complete
            LogPrintf("ArepacoinMiner : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }
        ...
        {
            LogPrintf("ArepacoinMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrintf("Running ArepacoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ArepacoinMiner terminated\n");
        throw;
    }
}

void GenerateArepacoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    minerThreads->create_thread(boost::bind(&ArepacoinStaker, pwallet));
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArepacoinMiner, pwallet));
}
